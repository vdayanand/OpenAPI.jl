...
## Installation
Place the Julia files generated under the `src` folder in your Julia project. Include deep.jl in the project code.
It would include the module named deep.

Implement the server methods as listed below. They are also documented with the deep module.
Launch a HTTP server with a router that has all handlers registered. A `register` method is provided in deep module for convenience.
